...
switch ($method) {
    case 'GET':
        try {
            ...
            $sql = "SELECT * FROM registration_works ORDER BY received_date DESC, seq_no DESC";
    ...
    case 'POST':
        try {
            ...
            if ($stmt->execute()) {
                ...
                echo json_encode(["status" => "error", "message" => "Unable to create registration work."]);
            }
        } catch (PDOException $e) {
            http_response_code(500);
            echo json_encode(["status" => "error", "message" => "Database error: " . $e->getMessage()]);
    ...
    case 'PUT':
        try {
            ...
            if (!empty($data->id)) {
                // Lockdown Check
                if (!empty($data->completion_date) && isLockdownActive($data->completion_date, 'registration')) {
                ...
                if (isset($data->subject)) {
                    // Full Update
                    $sql = "UPDATE registration_works SET 
                            seq_no = :seq_no,
                    ...
                    // Progress Type Update - Check setting before blocking type 2/3
                    $pType = (int) $data->progress_type;
                    if (in_array($pType, [2, 3])) {
                        $settingStmt = $conn->prepare("SELECT setting_value FROM system_settings WHERE setting_key = 'progress_type_assign'");
                        $settingStmt->execute();
                        $assignMode = $settingStmt->fetchColumn() ?: 'restricted';
                        if ($assignMode !== 'allowed') {
                            http_response_code(403);
                            echo json_encode(["status" => "error", "message" => "ไม่สามารถเปลี่ยนเป็นงานสุดขั้นตอนหรืองานศาลได้ (สามารถลดออกได้เท่านั้น)"]);
                            exit;
                        }
                    }
                    $sql = "UPDATE registration_works SET progress_type = :progress_type WHERE id = :id";
                    $stmt = $conn->prepare($sql);
    ...
    case 'DELETE':
        try {
            ...
            if (!empty($data->id)) {
                $sql = "DELETE FROM registration_works WHERE id = :id";
                $stmt = $conn->prepare($sql);
...
function logAction($action, $data)
{
    ...
    if ($action === 'ADD') {
        $details = "เพิ่มงานทะเบียนใหม่: " . ($data->subject ?? 'N/A');
    } elseif ($action === 'UPDATE') {
        if (isset($data->status_cause)) {
            $details = "อัปเดตสถานะงานทะเบียน (ID: $itemId) เป็น: " . $data->status_cause;
        } else {
            $details = "แก้ไขข้อมูลงานทะเบียน (ID: $itemId)";
        }
    } elseif ($action === 'DELETE') {
        $details = "ลบงานทะเบียน (ID: $itemId)";
    }

    Logger::log($userName, $action, 'registration', $itemId, $details);
}
?>